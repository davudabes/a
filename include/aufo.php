<?php
$aufo_link = mysqli_connect($DATABASE_HOST, $DATABASE_USER, $DATABASE_PASS, $DATABASE_NAME);
$aufo_query = "SELECT * FROM `all_coins` WHERE `coin_name` LIKE 'aufo' ORDER BY `all_coins`.`datetime` DESC LIMIT 1440";
$aufo_result = mysqli_query($aufo_link, $aufo_query);
/* fetch associative array */

while ($aufo_row = mysqli_fetch_row($aufo_result)) {
    $aufo_dates .= '"'.$aufo_row[0]. '",';
    $aufo_price .= $aufo_row[1]. ","; 
}
...
<canvas id="aufo_Chart" style="width: 600px;max-width: 600px;display: block;height: 300px;margin: 0 auto;"></canvas>

<script>
var php_dates = '<?php echo $aufo_dates; ?>';
var splitdates = php_dates.split(",")
var php_price = "<?php echo $aufo_price; ?>";
  ...
  options: {
    scales: {
        ...
        yAxes: [{
            ...
            ticks: {
                min: Math.min.apply(this, splitprice) - 0.00000000000005,
                max: Math.max.apply(this, splitprice) + 0.00000000000005,
            }
        }]
    },      
