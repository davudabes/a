<?php
$arbitmoon_link = mysqli_connect($DATABASE_HOST, $DATABASE_USER, $DATABASE_PASS, $DATABASE_NAME);
$arbitmoon_query = "SELECT * FROM `all_coins` WHERE `coin_name` LIKE 'arbitmoon' ORDER BY `all_coins`.`datetime` DESC LIMIT 1440";
$arbitmoon_result = mysqli_query($arbitmoon_link, $arbitmoon_query);
/* fetch associative array */

while ($arbitmoon_row = mysqli_fetch_row($arbitmoon_result)) {
    $arbitmoon_dates .= '"'.$arbitmoon_row[0]. '",';
    $arbitmoon_price .= $arbitmoon_row[1]. ","; 
}
...
<canvas id="arbitmoon_Chart" style="width: 600px;max-width: 600px;display: block;height: 300px;margin: 0 auto;"></canvas>

<script>
var php_dates = '<?php echo $arbitmoon_dates; ?>';
var splitdates = php_dates.split(",")
var php_price = "<?php echo $arbitmoon_price; ?>";
