<?php
$honeypot_link = mysqli_connect($DATABASE_HOST, $DATABASE_USER, $DATABASE_PASS, $DATABASE_NAME);
$honeypot_query = "SELECT * FROM `all_coins` WHERE `coin_name` LIKE 'honeypot' ORDER BY `all_coins`.`datetime` DESC LIMIT 1440";
$honeypot_result = mysqli_query($honeypot_link, $honeypot_query);
/* fetch associative array */

while ($honeypot_row = mysqli_fetch_row($honeypot_result)) {
    $honeypot_dates .= '"'.$honeypot_row[0]. '",';
    $honeypot_price .= $honeypot_row[1]. ","; 
}
...
?>
<script>
    var ppprice = "<?php echo $honeypot_price; ?>";
...
<canvas id="honeypot_Chart" style="width: 600px;max-width: 600px;display: block;height: 300px;margin: 0 auto;"></canvas>

<script>
var php_dates = '<?php echo $honeypot_dates; ?>';
var splitdates = php_dates.split(",")
var php_price = "<?php echo $honeypot_price; ?>";
