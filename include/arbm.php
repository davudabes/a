<?php
$arbm_link = mysqli_connect($DATABASE_HOST, $DATABASE_USER, $DATABASE_PASS, $DATABASE_NAME);
$arbm_query = "SELECT * FROM `all_coins` WHERE `coin_name` LIKE 'arbm' ORDER BY `all_coins`.`datetime` DESC LIMIT 1440";
$arbm_result = mysqli_query($arbm_link, $arbm_query);
/* fetch associative array */

while ($arbm_row = mysqli_fetch_row($arbm_result)) {
    $arbm_dates .= '"'.$arbm_row[0]. '",';
    $arbm_price .= $arbm_row[1]. ","; 
}
...
?>
<script>
    var ppprice = "<?php echo $arbm_price; ?>";
...
<canvas id="arbm_Chart" style="width: 600px;max-width: 600px;display: block;height: 300px;margin: 0 auto;"></canvas>

<script>
var php_dates = '<?php echo $arbm_dates; ?>';
var splitdates = php_dates.split(",")
var php_price = "<?php echo $arbm_price; ?>";
