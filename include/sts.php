<?php
$sts_link = mysqli_connect($DATABASE_HOST, $DATABASE_USER, $DATABASE_PASS, $DATABASE_NAME);
$sts_query = "SELECT * FROM `all_coins` WHERE `coin_name` LIKE 'sts' ORDER BY `all_coins`.`datetime` DESC LIMIT 1440";
$sts_result = mysqli_query($sts_link, $sts_query);
/* fetch associative array */

while ($sts_row = mysqli_fetch_row($sts_result)) {
    $sts_dates .= '"'.$sts_row[0]. '",';
    $sts_price .= $sts_row[1]. ","; 
}
...
?>
<script>
    var ppprice = "<?php echo $sts_price; ?>";
...
<canvas id="sts_Chart" style="width: 600px;max-width: 600px;display: block;height: 300px;margin: 0 auto;"></canvas>

<script>
var php_dates = '<?php echo $sts_dates; ?>';
var splitdates = php_dates.split(",")
var php_price = "<?php echo $sts_price; ?>";
