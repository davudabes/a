<?php
$arbiapu_link = mysqli_connect($DATABASE_HOST, $DATABASE_USER, $DATABASE_PASS, $DATABASE_NAME);
$arbiapu_query = "SELECT * FROM `all_coins` WHERE `coin_name` LIKE 'arbiapu' ORDER BY `all_coins`.`datetime` DESC LIMIT 1440";
$arbiapu_result = mysqli_query($arbiapu_link, $arbiapu_query);
/* fetch associative array */

while ($arbiapu_row = mysqli_fetch_row($arbiapu_result)) {
    $arbiapu_dates .= '"'.$arbiapu_row[0]. '",';
    $arbiapu_price .= $arbiapu_row[1]. ","; 
}
...
<canvas id="arbiapu_Chart" style="width: 600px;max-width: 600px;display: block;height: 300px;margin: 0 auto;"></canvas>

<script>
var php_dates = '<?php echo $arbiapu_dates; ?>';
var splitdates = php_dates.split(",")
var php_price = "<?php echo $arbiapu_price; ?>";
