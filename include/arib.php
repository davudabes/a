<?php
$arib_link = mysqli_connect($DATABASE_HOST, $DATABASE_USER, $DATABASE_PASS, $DATABASE_NAME);
$arib_query = "SELECT * FROM `all_coins` WHERE `coin_name` LIKE 'arib' ORDER BY `all_coins`.`datetime` DESC LIMIT 1440";
$arib_result = mysqli_query($arib_link, $arib_query);
/* fetch associative array */

while ($arib_row = mysqli_fetch_row($arib_result)) {
    $arib_dates .= '"'.$arib_row[0]. '",';
    $arib_price .= $arib_row[1]. ","; 
}
...
?>
<script>
    var ppprice = "<?php echo $arib_price; ?>";
...
<canvas id="arib_Chart" style="width: 600px;max-width: 600px;display: block;height: 300px;margin: 0 auto;"></canvas>

<script>
var php_dates = '<?php echo $arib_dates; ?>';
var splitdates = php_dates.split(",")
var php_price = "<?php echo $arib_price; ?>";
