<?php
$arbys_link = mysqli_connect($DATABASE_HOST, $DATABASE_USER, $DATABASE_PASS, $DATABASE_NAME);
$arbys_query = "SELECT * FROM `all_coins` WHERE `coin_name` LIKE 'arbys' ORDER BY `all_coins`.`datetime` DESC LIMIT 1440";
$arbys_result = mysqli_query($arbys_link, $arbys_query);
/* fetch associative array */

while ($arbys_row = mysqli_fetch_row($arbys_result)) {
    $arbys_dates .= '"'.$arbys_row[0]. '",';
    $arbys_price .= $arbys_row[1]. ","; 
}
...
?>
<script>
    var ppprice = "<?php echo $arbys_price; ?>";
...
<canvas id="arbys_Chart" style="width: 600px;max-width: 600px;display: block;height: 300px;margin: 0 auto;"></canvas>

<script>
var php_dates = '<?php echo $arbys_dates; ?>';
var splitdates = php_dates.split(",")
var php_price = "<?php echo $arbys_price; ?>";
