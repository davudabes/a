<?php
$gmx_link = mysqli_connect($DATABASE_HOST, $DATABASE_USER, $DATABASE_PASS, $DATABASE_NAME);
$gmx_query = "SELECT * FROM `all_coins` WHERE `coin_name` LIKE 'gmx' ORDER BY `all_coins`.`datetime` DESC LIMIT 1440";
$gmx_result = mysqli_query($gmx_link, $gmx_query);
/* fetch associative array */

while ($gmx_row = mysqli_fetch_row($gmx_result)) {
    $gmx_dates .= '"'.$gmx_row[0]. '",';
    $gmx_price .= $gmx_row[1]. ","; 
}
...
?>
<script>
    var ppprice = "<?php echo $gmx_price; ?>";
...
<canvas id="gmx_Chart" style="width: 600px;max-width: 600px;display: block;height: 300px;margin: 0 auto;"></canvas>

<script>
var php_dates = '<?php echo $gmx_dates; ?>';
var splitdates = php_dates.split(",")
var php_price = "<?php echo $gmx_price; ?>";
